...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship,
    ...
    # Check if aliens are destroyed and replace them.
    if len(aliens) == 0:
...
def get_number_aliens_x(ai_settings, alien_width):
    """Determine the number of aliens that fit in a new row."""
...
def check_aliens_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            # Treat this the same as if this ship got hit.
...
def update_aliens(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    Check if fleet is at an edge,
      and then update the position of the alien fleet.
    """
